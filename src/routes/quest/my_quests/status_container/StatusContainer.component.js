...
import QuestRow from '../quest_row/QuestRow.component';
...
class StatusContainer extends Component {
  ...
  componentWillReceiveProps(nextProps) {
    const willNeedPagination = this.doesNeedPagination(nextProps.quests, this.state.pagination.quantity);
    const state = {
      ...this.state,
      pagination: {
  ...
  increasePaginationQuantity(increase, dirtySet=false) {
    ...
    needsPagination = this.doesNeedPagination(quests, quantity);
    const state = {
                    ...this.state,
                    pagination: {
                      ...this.state.pagination,
  ...
  doesNeedPagination(quests, quantity) {
    return quests.length > quantity;
  }
  ...
  expandToggleClicked() {
    ...
    sortQuests(quests, this.state.sorts);
    const questElements = quests.slice(0, this.state.pagination.quantity).map((quest) => {
			return (
				<QuestRow key={quest._id} quest={quest} />
			);
		});
