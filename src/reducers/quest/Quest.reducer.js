...
export default (state = defaultState, action) => {
	...
	switch(action.type) {
		...
		case COMMIT_DRAG_MOVE_SUBMITTED:
			return commitDragMoveSubmitted(state, payload);
		case COMMIT_ADD_MOVE_SUBMITTED:
			return commitAddMoveSubmitted(state, payload);
    default:
      return state;
	}
};
...
function commitDragMoveSubmitted(state, { oldIndex, newIndex }) {
	let questContent = state.newQuest.content;
...
function commitAddMoveSubmitted(state, { content, index }) {
	state = contentAddSubmitted(state, { content, index });
